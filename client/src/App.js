...
function App() {
  ...
  const getGenre = () => {
    Axios.get("http://localhost:3004/genrefinder/" + searchGenres).then(
      (response) => {
        console.log(response.data);
        setMediaList(response.data);
      });
  ...
  const items = mediaList.map((val) => (
      <div key = {val.id} className="flex flex-row lg:flex-col mx-5 lg:mx-2 lg:mt-0 my-5 pb-5 items-center md:items-start lg:bg-white border-b-2 border-red-300 relative shadow-lg hover:bg-gray-50 hover:drop-shadow-lg">
        ...
        <a href={val.review_url} className="hover:cursor-pointer hover:drop-shadow-lg focus:ring focus:ring-violet-300">
          {<GetImage rev={val.review_url}></GetImage>}
        </a>
  ...
  return (
    <div className=".App">
      <TailwindButton>Show All Entertainment</TailwindButton>
  ...
  function GenreDropDown() {
    ...
    return (
      <div className="relative inline-flex gap-x-4">
        <button
          onClick= {getGenre}
          className="px-4 text-white bg-red-500 border-2 border-gray-200 rounded"
        >
          Genres
        </button>
        <svg
        ...
        <select
          ...
          onChange= {(e) => setSearchGenres(e.target.value)} value={searchGenres}
