...
function App() {
  ...
  const [searchGenre, setSearchGenre] = useState("Fantasy");
  const [mediaList, setMediaList] = useState([]);

  // Get all media dat from database call to backend server
  useEffect(() => {
      ...
      <div className="p-4 align-middle">
        <div className="mx-2 lg:w-[80%] lg:m-auto">
          <img
            ...
            alt="lorem ipsum"
            className="justify-center w-22 h-14"
          ></img>
...
// Get all Media (acts as a refresher from genres)
function SearchAllButton(props) {
    ...
    <div className="relative inline-flex gap-x-4">
      ...
      >
        ...
        {genreList.map((specificGenre, i) => {
          return <option key={i}>{specificGenre}</option>;
        })}
      </select>
    </div>
  );
...
function Items(props) {
  ...
  // Items is returned in card format
  const items = mediaList
    // Remove Parentheses (do this in the backend)
    .map((val) => ({
      ...val,
      genres: val.genres.replace("{", "").replace("}", ""),
    }))
    // Filter for multiple genres
    .filter((item) => {
      const hasGenre =
        searchGenre === "All Genres"
          ? true
          : item.genres.split(",").includes(searchGenre);
      const hasMediaType =
        searchMediaType === "All Media"
          ? true
          : item.media_type.includes(searchMediaType);
      return hasGenre && hasMediaType;
    })
    // Map genre and mediaType call to cards
    .map((val) => (
      <div
        key={val.id}
        className="flex flex-row lg:flex-col mx-5 lg:mx-2 lg:mt-0 my-5 pb-5 items-center md:items-start lg:bg-white border-b-2 border-red-300 relative shadow-lg hover:bg-gray-50 hover:drop-shadow-lg"
      >
        ...
        <a
          href={val.review_url}
          className="hover:cursor-pointer hover:drop-shadow-lg focus:ring focus:ring-violet-300"
        >
          <Image rev={val.review_url}></Image>
        </a>
        <div className="md:flex md:flex-col lg:px-3 gap-y-3 ">
          <h6 className="font-bold text-lg items-justify-center py-4">
            {val.name}
          </h6>
...
//Similar to Genre Drop Down filtered in Items
    ...
    <div className="relative inline-flex gap-x-4">
      ...
      >
        ...
        {mediaTypeList.map((specmedia, i) => {
          return <option key={i}>{specmedia}</option>;
        })}
      </select>
    </div>
  );
...
// Image webscraper from IGN review Articles
// Load time is slow
...
// Extraneous API calls utilized before new method of filtering
// Kept in case new API Call Methods to the database need to be utilized
/*
const getMedia = () => {
    Axios.get("http://localhost:3004/medias").then((response) => {
      console.log(response.data);
